...
const Listings = () => {
  ...
  const renderCard = (item) => {
    ...
    return (
      <div
        key={item._id}
        className="bg-[#013b5c] p-3 rounded-xl shadow-lg w-[220px] text-white flex flex-col items-center text-center hover:scale-105 hover:shadow-blue-600 transition-transform duration-200"
        onClick={()=>{navigate(`/project/${item._id}`)}}
      >
        <img
           src={`data:${item.image.contentType};base64,${bufferToBase64(item.image.data.data)}`}
          alt={item.name}
          className="rounded w-full h-[150px] object-cover mb-2"
          onError={(e) => { e.target.src = placeholderImage; }}
        />
        <h2 className="text-lg font-bold truncate mb-1">{item.name}</h2>
  ...
  return (
    <div className='min-h-screen w-screen bg-[#012F49] text-white px-4 overflow-x-hidden'>
      <h1 className='text-4xl mt-6 mb-4 ml-2'>
      ...
      <div className="flex justify-center gap-6 mb-6">
        <button
          ...
          onClick={() => {setViewType('lost'); setAttr("lostitem")}}
        >
          View Lost Items
        </button>
        <button
          ...
          onClick={() => {setViewType('found');setAttr("founditem")}}
        >
          View Found Items
        </button>
      </div>
      ...
      {viewType === 'lost' && (
        <div>
          <h2 className="text-3xl text-center mb-4">Lost Items</h2>
          {lostItems.length === 0 ? (
            <p className="text-center text-xl">No lost items found.</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 justify-center">
              {lostItems.map(renderCard)}
            </div>
          )}
        </div>
      )}
      ...
      {viewType === 'found' && (
        <div>
          <h2 className="text-3xl text-center mb-4">Found Items</h2>
          {foundItems.length === 0 ? (
            <p className="text-center text-xl">No found items found.</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 justify-center">
              {foundItems.map(renderCard)}
            </div>
          )}
        </div>
      )}
